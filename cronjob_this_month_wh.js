...
// select max(suba) - min(suba) from (SELECT mean("meter151/GasUsage1") AS suba FROM "data" WHERE time > now() -1d GROUP BY time(1d) fill(null))
 ...
 //   conn.connect();
    ...
    console.log("thisMonthDate : ", thisMonthDate)
        // 이번 달 전력량                                                                                  // +  %2B
    await fetch(`http://localhost:8086/query?pretty=true&db=emsdb&q=SELECT mean(\"meter152/Accura23501NetKwh\") %2B mean(\"meter153/Accura23501NetKwh\") FROM data where time > now() - ${thisMonthDate}d GROUP BY time(${thisMonthDate}d) fill(null)`, {
    ...
    .then(json => 
        {
            ...
            if (json === undefined || json === null){
                ...
                conn.query(`INSERT INTO stat_this_month_wh (created, rawData) VALUES                     
                ( now(), "${0}")`,
  ...
  async function lastFunction(){
    ...
    let thisMonthDate = new Date().getDate();
    console.log("thisMonthDate : ", thisMonthDate)
    await fetch(`http://localhost:8086/query?pretty=true&db=emsdb&q=SELECT DIFFERENCE(\*) from (SELECT mean(\"meter152/Accura23501NetKwh\") %2B mean(\"meter153/Accura23501NetKwh\") FROM data where time > now() - ${thisMonthDate}d GROUP BY time(${thisMonthDate}d) fill(null))`, {
    ...
    .then(json => 
        {
            ...
            if (json === undefined || json === null){
                ...
                conn.query(`UPDATE stat_this_month_wh SET delta = "${0}" WHERE id in (select id FROM (select max(id) as id FROM stat_this_month_wh) as temp)`,
                function (error, results, fields) {
                    if (error) {
                        console.log(error);
                    }
                ...
                conn.query(`UPDATE stat_this_month_wh SET delta = "${json.results[0].series[0].values[0][1]}" WHERE id in (select id FROM (select max(id) as id FROM stat_this_month_wh) as temp)`,
                function (error, results, fields) {
                    if (error) {
                        console.log(error);
                    }
